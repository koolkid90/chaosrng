...
ChaosRNG offers ~90-120 bits of practical security. Suitable for: passwords, session tokens, nonces, key generation for symmetric ciphers (when combined with a KDF). Not recommended for long-term cryptographic keys without additional hardening. Timing attacks are possible if an adversary can measure call intervals with high precision.
